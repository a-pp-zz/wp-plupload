(function($){$(function(){var uploadedFiles=[]
...
return translated}
var refreshCounter=function(counter,total,filelist_sel){$(filelist_sel).find('.plupload-uploaded').html(counter);if(total){$(filelist_sel).find('.plupload-total').html(total)}}
...
var pluploadInit=function(container){var data=$(container).data(),params={action:'plupload',_wpnonce:data.nonce},multipart_params={newname:data.name,types:data.types,dir:data.dir,ow:data.ow},browse_button=$(container).find('.plupload-pickfiles'),max_file_size_sel=$(container).find('.plupload-max-file-size'),allowed_formats_sel=$(container).find('.plupload-allowed-formats'),filelist_sel=$(container).find('.plupload-filelist'),total_sel=$(container).find('.plupload-total-holder'),total_uploaded=0,preview_sel=$(container).find('.plupload-preview'),features_sel=$(container).find('.plupload-features'),max_files_count=data.multi?Number(data.multi):1,silentmode=data.silentmode?Number(data.silentmode):0,receiver={};if(data.receiver){var receiver_obj=$(data.receiver)
...
$.each(files,function(i,file){showMessage(file,null,$(filelist_sel),'new')});if(max_files_count>1){$(total_sel).show();total_uploaded=0;refreshCounter(total_uploaded,files.length,total_sel)}
up.refresh();uploader.start()});uploader.bind('UploadProgress',function(up,file){var media_id='#media-item-o_'+file.id
...
return!1});$(".plupload-total-holder").on('click','.media-item a.dismiss',function(){$(this).parent().parent().parent().hide()
return!1});uploader.bind('UploadComplete',function(up,files){if(silentmode){$(features_sel).hide()}
if(receiver){$(window).trigger('wp-upload-complete',{id:receiver.obj.attr('id')})}
$(container).addClass('is-uploaded');if(receiver){if(receiver.tag=='input'||receiver.tag=='textarea'){if(uploadedFiles.length===0){receiver.obj.val('')}else{var json=JSON.stringify(uploadedFiles)
receiver.obj.val(json)}}}
uploadedFiles=[]})
uploader.bind('FileUploaded',function(up,file,info){var response=$.parseJSON(info.response);if(response.status===201){uploadedFiles.push(response.filename);total_uploaded++;refreshCounter(total_uploaded,0,total_sel);var media_id='#media-item-o_'+file.id;if(!silentmode){showMessage(file,'Файл успешно загружен!',$(filelist_sel),'success');if(max_files_count>5){setTimeout(function(){$(media_id).hide()},2000)}}else{$(media_id).hide()}
if(receiver.preview&&response.mime.search('image')!==-1){receiver.preview.css({maxWidth:receiver.preview_width+'px',maxHeight:receiver.preview_width+'px'});var img=$('<img />');img.attr('src',response.url);img.load(function(){if($(this).width()>=$(this).height()){img.attr('width',receiver.preview_width)}else{img.attr('height',receiver.preview_width)}});receiver.preview.html(img).show();$(container).trigger('wp-plupload-preview-changed',{preview:$(receiver.preview)})}}else{showMessage(file,response.message,$(filelist_sel),'error')}});uploader.init()}
