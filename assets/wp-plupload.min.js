...
var browse_button=$(container).find('.plupload-pickfiles');var max_file_size_sel=$(container).find('.plupload-max-file-size');var allowed_formats_sel=$(container).find('.plupload-allowed-formats');var filelist_sel=$(container).find('.plupload-filelist');var preview_sel=$(container).find('.plupload-preview');var receiver={}
...
var uploader=new plupload.Uploader({runtimes:'html5,html4',browse_button:$(browse_button).attr('id'),container:$(container).attr('id'),max_file_size:data.maxsize,url:pluploadConfig.ajaxurl+'?'+$.param(params),file_data_name:data.filefield,multi_selection:data.multi?!0:!1,multipart_params:multipart_params,filters:[{title:'Разрешенные типы',extensions:data.types}]})
