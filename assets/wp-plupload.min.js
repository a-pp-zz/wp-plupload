...
var pluploadInit=function(container){var data=$(container).data(),params={action:'plupload',_wpnonce:data.nonce},multipart_params={newname:data.name,types:data.types,dir:data.dir,ow:data.ow},browse_button=$(container).find('.plupload-pickfiles'),max_file_size_sel=$(container).find('.plupload-max-file-size'),allowed_formats_sel=$(container).find('.plupload-allowed-formats'),filelist_sel=$(container).find('.plupload-filelist'),preview_sel=$(container).find('.plupload-preview'),features_sel=$(container).find('.plupload-features'),max_files_count=data.multi?Number(data.multi):1,silentmode=data.silentmode?Number(data.silentmode):0,receiver={};if(data.receiver){var receiver_obj=$(data.receiver)
