...
var browse_button=$(container).find('.plupload-pickfiles');var max_file_size_sel=$(container).find('.plupload-max-file-size');var allowed_formats_sel=$(container).find('.plupload-allowed-formats');var filelist_sel=$(container).find('.plupload-filelist');var receiver={}
...
receiver.obj=receiver_obj}else{receiver=null}}
var uploader=new plupload.Uploader({runtimes:'html5,html4',browse_button:$(browse_button).attr('id'),container:$(container).attr('id'),max_file_size:data.maxsize,url:ajaxurl+'?'+$.param(params),file_data_name:data.filefield,multi_selection:data.multi?!0:!1,multipart_params:multipart_params,filters:[{title:'Разрешенные типы',extensions:data.types}]})
uploader.bind('Init',function(up,params){$(max_file_size_sel).html(data.maxsize);$(allowed_formats_sel).html(data.types);if(data.multi){$(container).find('.plupload-features').append('<div>Можно загрузить несколько файлов одновременно</div>')}});uploader.bind('FilesAdded',function(up,files){$(container).removeClass('is-uploaded')
$(filelist_sel).html('')
...
up.refresh();if(receiver){if(receiver.tag=='input'||receiver.tag=='textarea'){receiver.obj.val('')}else{receiver.obj.html('')}}});$(".plupload-filelist").on('click','.media-item a.dismiss',function(){$(this).parent().parent().remove()
...
receiver.obj.val(json)}}else if(data.type=='image'&&uploadedFiles.length===1){var img='<img src="/'+response.filename+'" />'
...
showMessage(file,'Файл успешно загружен!',$(filelist_sel),'success')}else{showMessage(file,response.message,$(filelist_sel),'error')}});uploader.init()}
